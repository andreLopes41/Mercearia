...
class ProdutoController:
    ...
    def criar_produto(
        self, nome: str, preco: float, categoria: Categoria
    ) -> None:
    ...
    def buscar_produtos_por_faixa_preco(
        self, preco_min: float, preco_max: float
    ) -> list:
        ...
        produtos: list = self.repository.read()
        return [
            pro for pro in produtos if preco_min <= pro['preco'] <= preco_max
        ]
    ...
    def atualizar_preco(self, nome: str, novo_preco: float) -> None:
        ...
        self.atualizar_produto(nome, novo_preco, produto_atual['categoria'])
