...
class VendaController:
    ...
    def realizar_venda(
        ...
        if not self.estoque_controller.verificar_disponibilidade(
            produto, quantidade
        ):
        ...
        nova_venda: Venda = Venda(
            produto, quantidade, comprador, vendedor, datetime.datetime.now()
        )
    ...
    def buscar_vendas_por_periodo(
        ...
        return [
            vnd for vnd in vendas if data_inicio <= vnd['data'] <= data_fim
        ]
    ...
    def buscar_vendas_por_cliente(self, cpf_cliente: str) -> list:
        ...
        vendas: list = self.repository.read()
        return [
            vnd for vnd in vendas if vnd['comprador']['cpf'] == cpf_cliente
        ]
    ...
    def buscar_vendas_por_vendedor(self, cpf_vendedor: str) -> list:
        ...
        vendas: list = self.repository.read()
        return [
            vnd for vnd in vendas if vnd['vendedor']['cpf'] == cpf_vendedor
        ]
    ...
    def buscar_vendas_por_produto(self, nome_produto: str) -> list:
        ...
        return [
            vnd
            for vnd in vendas
            if vnd['produto']['nome'].lower() == nome_produto.lower()
        ]

    def calcular_total_vendas_periodo(
        self, data_inicio: datetime, data_fim: datetime
    ) -> float:
        ...
        """
        vendas_periodo: list = self.buscar_vendas_por_periodo(
    ...
    def gerar_relatorio_vendas_diario(self, data: datetime = None) -> dict:
        ...
        inicio_dia: datetime = datetime.datetime(
